...
module tb_msic_tpg_verilog;
    ...
    always #10 if (clk1_en) clk1 = ~clk1; // 20ns period (CLK1 for LFSR)
    always #2  clk2 = ~clk2;              // 4ns  period (CLK2 for Johnson counter)
    ...
    initial begin
        $dumpfile("dump.vcd");
        $dumpvars(0, tb_msic_tpg_verilog);
    end
    ...
    initial begin
        ...
        bad_out = correct_out ^ 8'b00000001; // single-bit fault
